		...
		for(j=0;j<15;j++){
			...
			if(f>0){
				...
				if(value_of_everypoint[i][j].forbidden_moves==true)
					...
					if(pboard->gamemode.level==1)//如果是简单模式
						value_of_everypoint[i][j].value=100;//活三两端随机落子 
					...
					if(pboard->number<=10){
						if(pboard->number<5)
							...
							if(value_of_everypoint[i][j].forbidden_moves==true)
								...
								value_of_everypoint[i][j].count_of_sleepthree;
								if(pboard->gamemode.level==1)//简单模式
								value_of_everypoint[i][j].value=100+
...
void playing(menu mode){
	...
	if(mode.player!=-1){						//如果不是复盘 
		...
		if(board1.gamemode.player!=1)//如果不是人机 
			board1.gamemode.level=2;//提示的AI难度为困难 
		board1.number=0;
		int i,j;
		for(j=0;j<15;j++)
			for(i=0;i<15;i++){
